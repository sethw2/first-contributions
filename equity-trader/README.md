...
- **Edge comes from direction, not sizing.** Signal strength only *ranks and
  selects* trades; it never changes position size. Sizing is **equal-risk** (each
  trade risks a fixed % of equity to its stop), then clipped by the 20% notional
  cap and a **1% ADV participation cap** so a position in a thinner name can
  always be exited.
- **Volume is optional, not required.** Relative volume (RVOL) can nudge a
  candidate's ranking, but it never gates a signal — a valid breakout stands on
  price alone. Set `require_volume_confirmation: true` to opt into a hard volume
  gate; it's off by default.
